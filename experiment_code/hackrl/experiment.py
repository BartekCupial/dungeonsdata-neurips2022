...
def create_optimizer(model):
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if pn.endswith('bias'):
                ...
                no_decay.add(fpn)
            elif m._get_name() == "RecursiveScriptModule" and pn == "weight":
                decay.add(fpn)
    ...
    optim_groups = [
        {"params": [param_dict[pn] for pn in sorted(list(decay))], "weight_decay": FLAGS.weight_decay},
        {"params": [param_dict[pn] for pn in sorted(list(no_decay))], "weight_decay": 0.0},
    ]
    optimizer = torch.optim.AdamW(optim_groups, lr=FLAGS.adam_learning_rate, betas=(FLAGS.adam_beta1, FLAGS.adam_beta2))
    return optimizer
