...
appo_aa_bc_t = Path("/net/pr2/projects/plgrid/plgggmum_crl/bcupial/old_scratch/nle/12_05-09_03-nervous_wiles")

save_root_path = Path("/net/pr2/projects/plgrid/plgggmum_crl/bcupial/gamesavedir")

cpaths = []
for ckpt_paths in [
    get_checkpoint_paths(appo_aa_bc_t)
]:
    paths = []
...
folders = []
for i in range(2, 12):  # todo 10, 11
    folder = f"saves{i}"
...
params_grid = [
    {
        ...
        "gameloaddir": [spaths],
        "name": [f"{name}_{folder}"], # dig into level number
    }
    for paths in cpaths
